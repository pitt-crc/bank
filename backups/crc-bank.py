...
def check_insert_item_in_table(table, account):
    if not table.find_one(account=account) is None:
        exit("ERROR: Item already exists in database, did you want to modify it?")
...
def check_item_in_table(table, account, mode):
    if table.find_one(account=account) is None:
        if mode == 'modify' or mode == 'check':
            exit("ERROR: Item doesn't exists in database, did you want to insert it?")
        elif mode == 'reset_usage':
            exit("ERROR: Item doesn't exists in database, you should create a limit before resetting?")
...
if arguments['insert']:
    ...
    table.insert(dict(account=arguments['<account>'], su_limit_hrs=service_units,
                 date=date.today(), percent_informed=False))
    ...
    table.update(dict(account=arguments['<account>'], su_limit_hrs=service_units,
                 date=date.today(), percent_informed=False), ['account'])
    ...
    table.update(dict(account=arguments['<account>'], su_limit_hrs=service_units,
                 percent_informed=False), ['account'])
    ...
    if limit != -1 and int(raw_usage) > limit:
        ...
        command = "sacctmgr modify account where account={0} cluster={1} set GrpTresRunMins=cpu=0"
        for cluster in ['smp', 'gpu', 'mpi']:
        ...
        if limit != 0:
            ...
            Your allocation on H2P has run out of SUs. The one-year allocation started on {0}. Try `crc-usage.pl` to check your SU usage. To submit a proposal see http://core.sam.pitt.edu/beyond10K for more details.
            ...
            email["Subject"] = "Your allocation on H2P"
            email["From"] = From
            email["To"] = To
        ...
        if not informed:
            ...
            Your allocation on H2P is at {0}% usage. The one-year allocation started on {1}. Try `crc-usage.pl` to check your remaining SUs. To submit a proposal see http://core.sam.pitt.edu/beyond10K for more details.
    ...
    # Reset sshare usage
    command = "sacctmgr modify account where account={0} cluster={1} set RawUsage=0"
    for cluster in ['smp', 'gpu', 'mpi']:
        popen(command.format(arguments['<account>'], cluster))

    # Update the date in the database
    table.update(dict(account=arguments['<account>'], date=date.today(), percent_informed=False), ['account'])
    ...
    if comparison_days.days > 365:
        ...
        if limit == -1 or limit == 0:
            table.update(dict(account=arguments['<account>'], date=date.today(), percent_informed=False),
                        ['account'])
    ...
    if int(raw_usage) < limit:
        ...
        command = "sacctmgr modify account where account={0} cluster={1} set GrpTresRunMins=cpu=-1"
        for cluster in ['smp', 'gpu', 'mpi']:
            popen(command.format(arguments['<account>'], cluster))
    ...
    if delta.days == 90 and limit != -1:
        ...
        Your proposal on cluster H2P will reset and 10K SUs will be added to your account on {0}. Try `crc-usage.pl` to check your remaining SUs. To submit another proposal see http://core.sam.pitt.edu/beyond10K for more details.
