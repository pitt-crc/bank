...
class SignatureMatchesCLI(TestCase, CLIAsserts):
    ...
    def test_create_proposal(self) -> None:
        ...
        self.assert_parser_matches_func_signature(self.parser, f'create {settings.test_accounts[0]}')
        ...
        with self.assertRaises(SlurmAccountNotFoundError):
            self.assert_parser_matches_func_signature(
                self.parser,
                f'create {settings.nonexistent_account} --{settings.test_cluster} 100')

        # Create proposal, adding SUs to a specific cluster
        self.assert_parser_matches_func_signature(
            self.parser,
            f'create {settings.test_accounts[0]} --{settings.test_cluster} 100')
        ...
        self.assert_parser_matches_func_signature(
            self.parser,
            f'create {settings.test_accounts[0]} --start {start_date_str}')

        # Create a proposal, providing a custom start date with the wrong format
        with self.assertRaises(SystemExit):
            self.assert_parser_matches_func_signature(
                self.parser,
                f'create {settings.test_accounts[0]} --{settings.test_cluster} 100 --start 09/01/2500')
        ...
        self.assert_parser_matches_func_signature(
            self.parser,
            f'create {settings.test_accounts[0]} --{settings.test_cluster} 100 --end {end_date_str}')

        # Create a proposal, providing a custom end date with the wrong format
        with self.assertRaises(SystemExit):
            self.assert_parser_matches_func_signature(
                self.parser,
                f'create {settings.test_accounts[0]} --{settings.test_cluster} 100 --end 09/01/2500')
    ...
    def test_delete_proposal(self) -> None:
        ...
        self.assert_parser_matches_func_signature(self.parser, f'delete {settings.test_accounts[0]} --ID 0')
    ...
    def test_add_service_units(self) -> None:
        ...
        self.assert_parser_matches_func_signature(
            self.parser,
            f'add_sus {settings.test_accounts[0]} --{settings.test_cluster} 100'
        )

        # Add SUs to a specific proposal, usable on a specific cluster
        self.assert_parser_matches_func_signature(
            self.parser,
            f'add_sus {settings.test_accounts[0]} --ID 0 --{settings.test_cluster} 100'
        )
    ...
    def test_subtract_service_units(self) -> None:
        ...
        self.assert_parser_matches_func_signature(
            self.parser,
            f'subtract_sus {settings.test_accounts[0]} --{settings.test_cluster} 100'
        )
        ...
        self.assert_parser_matches_func_signature(
            self.parser,
            f'subtract_sus {settings.test_accounts[0]} --ID 0 --{settings.test_cluster} 100'
        )
    ...
    def test_modify_proposal_date(self) -> None:
        ...
        self.assert_parser_matches_func_signature(self.parser, f'modify_date {settings.test_accounts[0]} --start {date}')

        # Modify a specific proposal's date, changing only the start date
        self.assert_parser_matches_func_signature(
            self.parser,
            f'modify_date {settings.test_accounts[0]} --ID 0 --start {date}'
        )

        # Modify the active proposal's date, changing only the end date
        self.assert_parser_matches_func_signature(self.parser, f'modify_date {settings.test_accounts[0]} --end {date}')

        # Modify a specific proposal's date, changing only the end date
        self.assert_parser_matches_func_signature(
            self.parser,
            f'modify_date {settings.test_accounts[0]} --ID 0 --end {date}'
        )

        # Modify the active proposal's date, changing the start and end dates
        self.assert_parser_matches_func_signature(
            self.parser,
            f'modify_date {settings.test_accounts[0]} --start {date} --end {date}'
        )

        # Modify a specific proposal's date, changing the start and end dates
        self.assert_parser_matches_func_signature(
            self.parser,
            f'modify_date {settings.test_accounts[0]} --ID 0 --start {date} --end {date}'
        )
