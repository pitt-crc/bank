...
class Account:
    ...
    def _get_proposals(self, raise_missing: bool = False) -> Tuple[Proposal, List[Investor]]:
        """Return any proposals associated with the account

        Args:
            raise_missing: Raise an error if there is no proposal associated with the account
        ...
        Raises:
            MissingProposalError: If ``raise_missing`` is true and there is no primary proposal
        """
        ...
        if raise_missing and proposal is None:
...
def check_proposal_violations() -> None:
    ...
    for proposal in proposals:
        account = Account(bank.cli.parser.account)
